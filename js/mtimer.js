...
(function ($) {
    $.fn.mtimer = function(opts){
		var defaults = {
			...
			dateNum : 10,				// 天数
			timeStart : 9,				// 开始时刻
			timeNum : 12,				// 小时数(从开始到结束的总时长)
			onOk : null,				// 点击确定的回调函数
			onCancel : null,			// 点击取消的回调函数
			interval : 30,				// 时间间隔(单位秒)
		};
		...
		var picker = {
			init : function(){
				...
				for(var i=0,date_len = option.dateNum; i< date_len; i++){
				...
				var is_leg = in_array(interval,legal_interval);
				interval = (interval < 10) ? ('0' + interval) : interval;
				if(!is_leg) {
					alert('时间间隔参数有误...');
					return;
				}
				var loop_num = 60 / parseInt(interval);
				for(var j=0,time_len = option.timeNum; j < time_len; j++){
					var t = option.timeStart + j,
						sel = j == 0 ? 'selected' : '';
						t = (t < 10) ? ('0' + t) : t;
						for(var loop_i = 0;loop_i < loop_num;loop_i++) {
							var second_num = loop_i*interval;
							if(second_num !== 60) {
								second_num = (second_num < 10) ? ('0' + second_num) : second_num;
								timeStr += '<li class="'+sel+'" data-time="'+t+':'+second_num+'">'+t+':'+second_num+'</li>';
							}else {
